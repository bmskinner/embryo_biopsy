...
plot.biopsy.aneuploidy <- function(n.biopsy.aneuploid, biopsy.size, pct.embryo.aneuploid) {
  ...
  ggplot(biopsy.data, aes(x = pct.biopsy.aneuploid, y = PctBiopsies)) +
    ...
    labs(
      x = "Percent aneuploid cells in biopsy",
...
for (b in BIOPSY.SIZES) {
  ...
  agg.values.merge2.data <- raw.values %>%
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(
      EmbryoClass = to.merged.class.2(Aneuploidy),
  ...
  ggsave(combo.pgdis.plot,
    filename = paste0(FIGURE.OUTPUT.DIR, "/predictive_heatmap_pgdis_combined_", b, ".svg"), dpi = 300, units = "mm",
    width = 170, height = 85
  )
  ggsave(combo.pgdis.plot,
    filename = paste0(FIGURE.OUTPUT.DIR, "/predictive_heatmap_pgdis_combined_", b, ".png"), dpi = 300, units = "mm",
    width = 170, height = 85
  )

  combo.merge.plot <- hmap.merge.plot + col.merge.plot + plot_annotation(tag_levels = c("A"))
  ggsave(combo.merge.plot,
    filename = paste0(FIGURE.OUTPUT.DIR, "/predictive_heatmap_merge_combined_", b, ".svg"), dpi = 300, units = "mm",
    width = 170, height = 85
  )
  ggsave(combo.merge.plot,
    filename = paste0(FIGURE.OUTPUT.DIR, "/predictive_heatmap_merge_combined_", b, ".png"), dpi = 300, units = "mm",
    width = 170, height = 85
  )

  combo.merge2.plot <- hmap.merge2.plot + col.merge2.plot + plot_annotation(tag_levels = c("A"))
  ggsave(combo.merge2.plot,
    filename = paste0(FIGURE.OUTPUT.DIR, "/predictive_heatmap_merge2_combined_", b, ".svg"), dpi = 300, units = "mm",
    width = 170, height = 85
  )
  ggsave(combo.merge2.plot,
    filename = paste0(FIGURE.OUTPUT.DIR, "/predictive_heatmap_merge2_combined_", b, ".png"), dpi = 300, units = "mm",
    width = 170, height = 85
  )
}

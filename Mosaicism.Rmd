...
```{r output-aneuploidy-step, echo=FALSE, message=F}
aneuploidy.step.plot = plot.step(result, "Aneuploidy", seq(0, 0.5, 0.1))
...
```{r output-dispersal-step, echo=FALSE, message=F}
dispersal.step.plot = plot.step(result, "Dispersal", combinations$dispersals)
...
dispersal.aneuploidy.heatmap = ggplot(as.data.frame(result), aes(x = aneuploidy*100, y = dispersal, fill = accuracy)) +
  geom_tile() +
  scale_fill_viridis_c(breaks = seq(0, 100, 20)) +
  labs(x = "Percent aneuploidy in embryo", 
       y = "Dispersal of aneuploid cells",
  ...
  scale_y_continuous(breaks = seq(0, 1, 0.1)) +
  theme_classic()
save.data(dispersal.aneuploidy.heatmap, "dispersal.aneuploidy.heatmap")
```
...
```{r output-biopsy-size-plot, echo=FALSE, message=F}
biopsy.size.step.plot = plot.step(result, "Biopsy size", combinations$samples)
save.data(biopsy.size.step.plot, "biopsy.size.step.plot")
...
ggplot(constrain.matching.biopsies %>% dplyr::filter(n.aneuploid==1), aes(x=aneuploidy, y=dispersal, fill=Pct)) +
  ...
  labs(x = "Aneuploidy", 
       ...
       fill = "Percentage\nof biopsies") + 
  theme_classic()
ggsave("Constrained_dispersal.png", width=85, height=85, units="mm", dpi=300)
...
hyperg = do.call(rbind, lapply(seq(0.1, 0.9, 0.1), make.hyper))
...
compare.hyper.to.actual = function(prop.aneuploid){
  ...
  # actual.plot = plot.biopsy.aneuploidy(b, 5, 20)
...
ggplot(biopsy.data, aes(x=pct.biopsy.aneuploid, y = PctBiopsies)) +
  geom_col() +
  geom_line(data = hyperg, aes(x = n.aneuploid, y = probability), 
...
```{r validate-cluster-calc, eval=T, echo=FALSE, warning=F, message=F, cache=T }

e10k.result = do.call(rbind, lapply(seq(0.1, 0.9, 0.1), function(prop.aneuploid){
  e10k = Embryo(n.cells = 10000, dispersal = 0, prop.aneuploid = prop.aneuploid)
...
ggplot(e10k.result, aes(x = n.aneuploid, y = Pct)) +
  geom_col()+
  labs(x = "Number of aneuploid cells in biopsy",
       y = "Percent of biopsies") +
  scale_y_continuous(breaks = seq(0, 100, 10)) +
  ...
  coord_cartesian(ylim = c(0, 100)) + 
  facet_wrap(~PctAneuploid, ncol = 3, labeller = labeller(PctAneuploid = pct.labels) ) + 
  theme_classic() +

...
for(n.cells in seq(150, 250, 50)){
  for(start in seq(1, max(1, n.replicates-9), 10)){ # replicates must be multiple of 10
    ...
    file = paste0("2x5_", n.cells, "cells_", n.replicates, "rep_", start, "-", end, ".csv")
    if(!file.exists(file)){
      cat("Creating", file, "\n")
...
result.2x5 = do.call(rbind, lapply(list.files(pattern = paste0("2x5_", n.cells, "cells_", n.replicates, "rep_.*.csv")), function(f){read.csv(f, header = T, stringsAsFactors = F)}))
...
calc.multicell.heatmap = function(embryo.size){
  ...
  result.2x5.150 = do.call(rbind, lapply(list.files(pattern = paste0("2x5_", embryo.size, "cells_", n.replicates, "rep_.*.csv")), function(f){read.csv(f, header = T, stringsAsFactors = F)}))

...
```{r, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap="PGDIS classes cannot be inferred from biopsy results" }
...
```{r, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap="Biopsy aneuploidy correlation with embryo for 1x10 biopsies", fig.height=5 }
combinations = expand.grid(aneuploidies = seq(0, 1, 0.05),
                           dispersals   = seq(0, 1, 0.05))

# The possible outcomes from all combinations of embryo parameters
result.1x10 = do.call(rbind, mcmapply(function(aneuploidy, dispersal){
...
result.1x10$PctBiopsyAneuploid = result.1x10$n.aneuploid / 10 * 100
...
```{r, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap="Correct PGDIS classes from 1x10 biopsy results" }
...
```{r, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap="Taking two five cell biopsies improves over one 10 cell biopsy"}
...
# p1 = plot.biopsy.aneuploidy(b, 5, 20)

# Get all pairwise combinations of biopsy result
# Ignore the issue of double sampling for now

g = as.data.frame(expand.grid(b, b)) %>%
  dplyr::mutate(Avg = (Var1+Var2)/2)

# p2 = plot.biopsy.aneuploidy(g$Avg, 5, 20)

# compare to the 10 cell biopsies from the same embryo
b10 = tessera::takeAllBiopsies(e, biopsy.size = 10, chromosome = 1)

# p3 = plot.biopsy.aneuploidy(b10, 10, 20)

p3 = plot.pgdis(b, 5) + labs(title = "1x5 cell")

p4 = plot.pgdis(g$Avg, 5) + labs(title = "2x5 cell")
# pgdis.accuracy(g$Avg, 5, 20)
...
```{r, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap="2x5 biopsies do not overcome the issues in interpreting biopsy aneuploidy "}
combinations = expand.grid(aneuploidies = seq(0, 1, 0.05),
                           dispersals   = seq(0, 1, 0.05))

# The possible outcomes from all combinations of embryo parameters
result.2x5 = do.call(rbind, mcmapply(function(aneuploidy, dispersal){
  ...
  b = tessera::takeAllBiopsies(e, biopsy.size = 5, chromosome = 1)
  g = (as.data.frame(expand.grid(b, b)) %>%
    dplyr::mutate(Avg = (Var1+Var2)/2))$Avg

  cbind(aneuploidy = rep(aneuploidy, length(g)),
        dispersal  = rep(dispersal, length(g)),
        n.aneuploid = g)
}, mc.cores = n.cores,
aneuploidy = combinations$aneuploidies,
dispersal  = combinations$dispersals,
SIMPLIFY = F)) %>% as.data.frame
result.2x5$PctBiopsyAneuploid = result.2x5$n.aneuploid / 5 * 100
...
```{r, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap="PGDIS classes cannot be reliably inferred from biopsy results" }
...
There is an improvement versus a 1x5 biopsy, but not clearly greater than that provided by a 1x10 biopsy, suggesting repeated biopsying is not an effective solution to the mosaicism problem.

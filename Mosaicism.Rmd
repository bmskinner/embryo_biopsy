...
```{r aneuploidy-pgdis, eval=T, echo=FALSE, warning=F, message=F, cache=F, fig.height=3, fig.show='hold', fig.cap="PGDIS classes for embryos with A) 10% (euploid) and B) 50% aneuploidy (high-level) demonstrate the difference in accuracy from changing aneuploidy level" }
...
```{r dispersal-pgdis-all, eval=T, echo=FALSE, warning=F, message=F, cache=F, fig.height=3, fig.show='hold', fig.cap= paste("PGDIS classification accuracy by dispersion level. Error bars show standard deviation across", N.REPLICATES, "replicates") }
...
```{r calc-2x5, eval=T, echo=FALSE, warning=F, message=F, cache=F}

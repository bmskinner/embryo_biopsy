...
```{r dispersal-pgdis, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap="PGDIS classes for embryos with 20% aneuploidy (low level) with A) low and B) high dispersal demonstrate the difference in accuracy from changing dispersal. In this case, higher dispersal yields a higher accuracy." }
...
```{r dispersal-pgdis-all, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap= paste("PGDIS classification accuracy by dispersion level. Error bars show standard deviation across", n.replicates, "replicates") }
...
```{r biopsy-size, eval=T, echo=FALSE, cache=T, fig.show='hold', fig.cap="Biopsy size does not drastically affect the accuracy of the biopsy beyond about 5% of the embryo"}
...
```{r biopsy-size-pgdis, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap=paste("Biopsy and embryo size variation does not greatly affect the model results with a biopsy sd of 1. Error bars show standard deviation across", n.replicates, "replicates")}
combinations = expand.grid(biopsy.sizes = seq(3, 50, 1),
                           seeds   = 1:n.replicates)
...
```{r full-possible-embryo-fig, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap="The probability space of 5-cell biopsies for a 200 cell embryo"}
...
```{r heatmap-1x5, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.height= 3, fig.cap="Confidently relating biopsy aneuploidy to embryo aneuploidy is only possible if dispersal is high. White boxes show PGDIS classification boundaries." }
...
```{r heatmap-1x10, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.height= 3, fig.cap="Biopsy aneuploidy correlation with embryo for 1x10 biopsies" }
combinations = expand.grid(aneuploidies = seq(0, 1, 0.05),
                           dispersals   = seq(0, 1, 0.05),
                           seeds        = 1:n.replicates)
...
take.two.biopsies = function(embryo, biopsy.size, chromosome){
  ...
  for(i in 1:length(embryo)){
    ...
    exclude.list = getNeighboursToExclude(embryo, i)
...
combinations = expand.grid(aneuploidies = seq(0, 1, 0.05),
                           dispersals   = seq(0, 1, 0.05),
                           seeds        = 42) # this takes too long with multiple seeds
...
```{r heatmap-2x5, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.height=9, fig.cap="A) 1x5 B) 1x10 C) 2x5. Taking two 5-cell biopsies does not overcome the issues in interpreting biopsy aneuploidy"}
...
The results of sampling a fully dispersed embryo should not follow a hypergeometric distribution (sampling without replacement), since the positions of aneuploid cells are not random. We can demonstrate that e.g. at 20% aneuploidy there are more biopsies with one aneuploid than would be expected for randomly assorted cells.

```{r validate-hyper, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.height=6, fig.width=7, fig.cap="At high dispersal, biopsy results (bars) are similar to but not precisely the hypergeometric distribution (green line)" }
...
ggplot(biopsy.data, aes(x=pct.biopsy.aneuploid, y = PctBiopsies)) +
  ...
  facet_wrap(~prop.aneuploid, ncol = 3) +
  theme_classic()



# (compare.hyper.to.actual(0.1) + compare.hyper.to.actual(0.2) + compare.hyper.to.actual(0.3)) /
#   (compare.hyper.to.actual(0.4) + compare.hyper.to.actual(0.5) + compare.hyper.to.actual(0.6)) /
#   (compare.hyper.to.actual(0.7) + compare.hyper.to.actual(0.8) + compare.hyper.to.actual(0.9)) + plot_annotation(tag_levels = c("A"))

```

At the other end of the dispersal scale, we can check that for an unrealistically large embryo of 10,000 cells, if the aneuploid cells are clustered, we should get predominantly fully euploid or aneuploid biopsies.

```{r validate-cluster, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.height=3, fig.width=7, fig.cap="At low dispersal, cells remain clustered" }
e10k = Embryo(n.cells = 10000, dispersal = 0)
b10k = data.frame(n.aneuploid = tessera::takeAllBiopsies(e10k, biopsy.size = 5, chromosome = 1)) %>%
  dplyr::group_by(n.aneuploid) %>%
  dplyr::summarise(Pct = dplyr::n()/100)

ggplot(b10k, aes(x = n.aneuploid, y = Pct)) +
  geom_col()+
  labs(x = "Number of aneuploid cells in biopsy",
       y = "Percent of biopsies") +
  scale_y_continuous(breaks = seq(0, 100, 10)) +

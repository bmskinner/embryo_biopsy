...
plot.embryo = function(e){
  ...
  plot_ly( type="scatter3d",
    ...
    add_annotations( text="Click and drag to rotate",
                     xref="paper", yref="paper",
                     x=0.0, xanchor="left",
                     y=1.0, yanchor="top",
                     legendtitle=TRUE, showarrow=FALSE ) %>%
    config(displayModeBar = FALSE, scrollZoom = F) %>%
    layout(scene = list(
...
#   "function(el, x) {
#   var gd = document.getElementById(el.id); 
#   Plotly.downloadImage(gd, {format: 'png', width: 720, height: 480, filename: 'e2'});
#   }"
#  )
# 
# # Restore viewer to old setting (e.g. RStudio)
...
```{r, echo=F, warning=F, message=F, cache=T, fig.show = 'hold', fig.cap="Accuracy is lowest when half the embryo is aneuploid"}
...
e2 = tessera::create.embryo(n.cells = 200,
                           prop.aneuploids = 0.5,
                           dispersions = 1,
                           concordance = 1, seed = 44)
b2 = tessera::take.all.biopsies(e2, n.cells.per.sample = 5, chromosome = 1)
...
```{r, echo=F, warning=F, message=F, cache=T, fig.show='hold', fig.cap="Biopsy accuracy is higher when aneuploid cells are dispersed"}
...
```{r, echo=FALSE, message=F, warning=F, cache=T, fig.show='hold', fig.cap="Embryos with varying dispersal of aneuploid cells"}
...
```{r, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap="Biopsy size affects accuracy"}
e = tessera::create.embryo(n.cells = 200, 
                           prop.aneuploids = 0.2, 
                           dispersions = 1, 
                           concordance = 1, seed = 42)
...
```{r, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap="Sampling more than 5% of the embryo is not more informative regardless of embryo size"}
...
```{r, eval=T, echo=FALSE, warning=F, message=F, cache=T, fig.show='hold', fig.cap="Liklihood of each PGDIS class producing a 5-cell biopsy with a given number of aneuploid cells" }
...
We can conclude from this that although there are limitations to biopsies, a 5-cell biopsy of a 200 cell embryo is able to give a useful indication as to the true level of aneuploidy of the embryo, but, as one would expect, biopsies have best accuracy at the extremes of low or high aneuploidy.

...
knitr::opts_chunk$set(echo = TRUE, warning = F, message = F, cache.lazy = F)
library(tessera)
library(magrittr)
library(dplyr)
library(plotly)
...
```{r output-embryo-combination-plot, echo=FALSE, message=F}
embryo.combination.plot = ggplot(as.data.frame(result), aes(x = n.aneuploid, fill = n.aneuploid==1)) + 
  geom_bar() + 
  labs(x = "Number of aneuploid cells in biopsy",
       y = "Number of possible biopsies\nfrom all embryo combinations") + 
  scale_x_continuous(breaks = seq(0, 5, 1)) +
  ...
  theme_classic() + 
  theme(legend.position = "none")
save.data(embryo.combination.plot, "embryo.combination.plot")
...
```{r output-embryo-combination-source-plot, echo=FALSE, message=F}
embryo.combination.source.plot = ggplot(matching.biopsies %>% dplyr::filter(n.aneuploid==1), aes(x=aneuploidy, y=dispersal, fill=Pct)) +
  geom_tile() +
  scale_fill_viridis_c(breaks = seq(0.1, 1.1, 0.2)) +
  labs(x = "Aneuploidy", 
       y = "Dispersal",
       fill = "Percentage\nof biopsies") + 
  theme_classic()
save.data(embryo.combination.source.plot, "embryo.combination.source.plot")
...
```{r full-possible-embryo-fig, eval=T, echo=FALSE, warning=F, message=F, cache=F, fig.show='hold', fig.cap="The probability space of 5-cell biopsies for a 200 cell embryo"}
...
```{r output-embryo-combination-all-plot, echo=FALSE, message=F}
embryo.combination.all.plot = ggplot(matching.biopsies, aes(x=aneuploidy, y=dispersal, fill=Pct)) +
  geom_tile() +
  scale_fill_viridis_c(breaks = seq(0.1, 1.1, 0.2)) +
  labs(x = "Aneuploidy", 
       y = "Dispersal",
  ...
  theme_classic() +
  facet_wrap(~n.aneuploid, ncol = 3, labeller = labeller(n.aneuploid = n.aneuploid.labels))
save.data(embryo.combination.all.plot, "embryo.combination.all.plot")
...
# TODO
 ...
 - add embryo size variability
 - sequencing depth consideration
